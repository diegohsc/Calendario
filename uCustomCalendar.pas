...
type
  ...
  private
    ...
    FSelectedDay: ShortInt;
    procedure SetSelectedDay(const Value: ShortInt);
  ...
  public
    ...
    procedure DayClick(Sender: TObject);
     procedure MarkerDay(day: integer);
     procedure setMonth(aData:string);
       function ResultDaysSelected: string;
  published
    ...
    property SelectedDay:  ShortInt read FSelectedDay write SetSelectedDay;
...
procedure TCustomCalendar.setMonth(aData: string);
begin
  FSelectedDate:= StrToDate(aData);
  FMonthName := GetMonthName(FSelectedDate);
end;

procedure TCustomCalendar.SetSelectedDay(const Value: ShortInt);
begin
  FSelectedDay := Value;
end;
...
begin
    ...
    c.Visible := true;
    c.Fill.Kind := TBrushKind.Solid;
    c.Fill.Color := $FF1A2FD1 ; //BackgroundColor;
    c.Tag := 1;
...
procedure TCustomCalendar.MarkerDay(day : integer );
var
    c : TCircle;
    lbl : TLabel;
begin
...
function TCustomCalendar.ResultDaysSelected( ):string;
var
  c : TCircle;
  lbl : TLabel;
  diasDoMes:Integer;
  I: Integer;
  ObjetoDia:TRectangle;
  ListaDeDias:string;
  virgula: ShortString;
begin

   diasDoMes:= DaysInMonth(FSelectedDate);
   ListaDeDias:='';
   virgula:='';
    for I:= 1 to diasDoMes do
    begin
      ObjetoDia := rectCalendar.FindComponent('day' +I.ToString) as TRectangle;
      c := ObjetoDia.FindComponent('day_c' + I.ToString) as TCircle;

      if c.tag = 1 then
      begin
        ListaDeDias:= ListaDeDias + virgula + I.ToString;
        virgula:= ' , ';
      end;
    end;
